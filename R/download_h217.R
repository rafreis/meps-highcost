# =============================================================================
# meps-highcost / R/download_h217.R
# Downloads the Panel 23 two-year longitudinal file (HC-217, 2018-2019) from
# AHRQ MEPS into data/raw/HC-217/, writes a checksum manifest, and verifies
# against R/config.R's PANEL_MAP (must be HC-217, never HC-236).
...
# AHRQ MEPS HC-217 (Panel 23 Longitudinal, 2018-2019), SAS transport (.ssp / .dat)
# Canonical AHRQ download page: https://meps.ahrq.gov/mepsweb/data_stats/download_data_files_detail.jsp?cboPufNumber=HC-217
# Stata .dta is the most robust single-file format to parse with haven and is
# offered directly by AHRQ for this file.
FILE_URL  <- "https://meps.ahrq.gov/mepsweb/data_files/pufs/h217/h217dta.zip"
ZIP_PATH  <- file.path(RAW_DIR, "h217dta.zip")
DTA_NAME  <- "h217.dta"
...
if (!file.exists(DTA_PATH)) {
  candidates <- list.files(RAW_DIR, pattern = "(?i)^h217.*\\.dta$", full.names = TRUE, recursive = TRUE)
...
if (!file.exists(DTA_PATH)) {
  stop(
    ...
    ". Manually place h217.dta (or h217.ssp) in ", RAW_DIR,
    " from https://meps.ahrq.gov/mepsweb/data_stats/download_data_files_detail.jsp?cboPufNumber=HC-217"
  )
}
